...
check_prerequisites() {
    ...
    if oc get secret pull-secret -n "$PROJECT_CPD_INST_OPERATORS" &> /dev/null; then
        log_success "pull-secret exists in operator namespace: $PROJECT_CPD_INST_OPERATORS"
    else
        log_error "pull-secret missing in operator namespace: $PROJECT_CPD_INST_OPERATORS"
        log_error "  Create the secret using: oc create secret docker-registry pull-secret --docker-server=cp.icr.io --docker-username=cp --docker-password=<entitlement-key> -n $PROJECT_CPD_INST_OPERATORS"
    fi
    
    # Check IBM Entitlement Key in operand namespace
    if oc get secret pull-secret -n "$PROJECT_CPD_INST_OPERANDS" &> /dev/null; then
        log_success "pull-secret exists in operand namespace: $PROJECT_CPD_INST_OPERANDS"
    else
        log_error "pull-secret missing in operand namespace: $PROJECT_CPD_INST_OPERANDS"
        log_error "  Create the secret using: oc create secret docker-registry pull-secret --docker-server=cp.icr.io --docker-username=cp --docker-password=<entitlement-key> -n $PROJECT_CPD_INST_OPERANDS"
    fi
