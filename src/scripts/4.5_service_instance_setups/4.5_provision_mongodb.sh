...
export MONGO_MANAGER_PAYLOAD_FILE="${SERVICE_INSTANCE_FILE_DIR}/mongodb-opsmanager-instance.json"

export MONGO_MANAGER_INSTANCE_NAME="mongodb-manager"
export MONGO_ADDON_VERSION="5.3.1"    # CPD addon version (from zen catalog)
VERSION="${MONGO_ADDON_VERSION}"
export MONGO_MANAGER_VERSION="8.0.6"  # MongoDB server version: "8.0.6" or "7.0.15"
...
# MongoDB Ops - Metadata Manager (appDB replica set - must be odd; 3 is the minimum for HA)
MD_INSTANCE_REPLICAS=3 # between 1 and 50 replicas
MD_REPLICA_CPU=1 # between 1 and 16 CPUs
MD_REPLICA_MEMORY=1 # between 1 and 64 GB
MD_STORAGE_SIZE=10 # between 1 and 300 GB Storage
# MongoDB Ops - Instance Manager
MONGO_MANAGER_INSTANCE_REPLICAS=2 # between 1 and 50 replicas
MONGO_MANAGER_REPLICA_CPU=2 # between 1 and 16 CPUs
MONGO_MANAGER_REPLICA_MEMORY=4 # between 1 and 64 GB
MONGO_MANAGER_STORAGE_SIZE=10 # between 1 and 1000 GB Storage

PREP_MONGODB_MANAGER='cat << EOF > ${MONGO_MANAGER_PAYLOAD_FILE}
{
    ...
    "parameters": {
        ...
        "mdunit": "Gi",
        "mdstorageclass": "${STG_CLASS_BLOCK}",
        ...
        "mgrunit": "Gi",
        "mgrstorageclass": "${STG_CLASS_BLOCK}",
...
echo "eval ${PREP_MONGODB_MANAGER}"
# cpd-cli service-instance status mongodb-manager --profile=cpadmin-profile  
...
if [[ "${SINGLE_MANAGER_SETUP}" == true ]]; then
    EXISTING_MANAGER_STATUS=$(cpd-cli service-instance status ${MONGO_MANAGER_INSTANCE_NAME} \
        ...
        --output=json 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('service_instance',{}).get('provision_status',''))" 2>/dev/null || echo "")

    if [[ "${EXISTING_MANAGER_STATUS}" == "PROVISIONED" || "${EXISTING_MANAGER_STATUS}" == "PROVISION_IN_PROGRESS" || "${EXISTING_MANAGER_STATUS}" == "PENDING" ]]; then
...
export MONGODB_PAYLOAD_FILE="${SERVICE_INSTANCE_FILE_DIR}/mongodb-cp4d-instance.json"
...
# MongoDB 
MONGO_INSTANCE_REPLICAS=3 # between 1 and 50 replicas
...
PREP_MONGODB_INSTANCE='cat << EOF > ${MONGODB_PAYLOAD_FILE}
{
    "addon_type": "mongodb",
...
echo "eval ${PREP_MONGODB_INSTANCE}"
