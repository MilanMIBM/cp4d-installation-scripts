...
#
# Column shapes.  Collection columns (LICENSES, CATALOGS, CERTIFICATIONS,
# CLASSIFICATIONS, ARCHITECTURES, K8S_DISTROS, MANAGED_PLATFORMS) are emitted
# as JSON, so they load with json.loads/ast.literal_eval rather than needing to
# be split on a delimiter; an empty one is [], not "".  Scalar columns stay
# plain strings.  LICENSES is an array of objects, one per license, each with
# the YAML key as license_name plus that license's own fields and its metadata
# flattened in alongside them; mediaType is dropped:
#   [{"license_name":"apache2","displayDescription":"Apache License, version
#     2.0","displayName":"Apache 2.0","ref":"LICENSE","url":"https://..."}]
#
# Only the CASE name is carried over from the input list: its "Latest Version"
# and "Latest App Version" columns duplicate LATEST_VERSION and APP_VERSION,
# which are read from the version.yaml.  KEEP_LIST_COLUMNS=1 keeps them.
...
# case.licenses as a JSON array.  Each license becomes one object carrying the
# YAML key as license_name, its own scalars (ref, url, ...) and the contents of
# its metadata block flattened in alongside them.  mediaType is dropped.
# Values may be folded across continuation lines, so a key stays "open" until
# the next key or the end of its block.
cat > "$TMP/licenses.awk" <<'AWK'
function esc(s) {
  gsub(/\\/, "\\\\", s); gsub(/"/, "\\\"", s)
  gsub(/\t/, " ", s); gsub(/\r/, " ", s)
  while (s ~ /  /) gsub(/  /, " ", s)
...
# Buffer the value being accumulated, then commit it as a JSON pair.
function flush(  v) {
  if (curkey == "") return
  v = esc(unq(curval))
  if (curkey != "mediaType")
    body = body (body == "" ? "" : ",") "\"" esc(curkey) "\":\"" v "\""
  curkey = ""; curval = ""
}
function closelic() {
  flush()
  if (lic == "") return
  out = out (out == "" ? "" : ",") "{\"license_name\":\"" esc(lic) "\"" (body == "" ? "" : "," body) "}"
  lic = ""; body = ""
}
BEGIN { incase = 0; inlic = 0 }
/^case:[[:space:]]*$/ { incase = 1; next }
/^[A-Za-z]/ { if (incase) closelic(); incase = 0; inlic = 0 }
incase && /^  licenses:[[:space:]]*$/ { inlic = 1; next }
incase && inlic && /^  [A-Za-z0-9_]+:/ { closelic(); inlic = 0 }
inlic {
  # A license key: four spaces of indent.
  if ($0 ~ /^    [^ ].*:[[:space:]]*$/ && $0 !~ /^    (mediaType|metadata|ref|url):/) {
    closelic()
    lic = $0; sub(/^    /, "", lic); sub(/:[[:space:]]*$/, "", lic); lic = unq(lic)
    next
  }
  if (lic == "") next
  # metadata: is a container; its children are flattened in at the same level.
  if ($0 ~ /^      metadata:[[:space:]]*$/) { flush(); next }
  # Scalars of the license (6 spaces) or of its metadata (8 spaces).
  ...
  # A continuation of the value opened above.
  if (curkey != "" && $0 ~ /^[[:space:]]+[^ ]/) {
    line = $0; sub(/^[[:space:]]+/, "", line)
    curval = curval " " line
    next
...
END { if (incase) closelic(); print "[" out "]" }
AWK
...
# Keep only the first column (the CASE name) of an input list row; the rest
# duplicate columns we re-derive from version.yaml.  Set KEEP_LIST_COLUMNS=1
# to carry the original list columns through unchanged.
KEEP_LIST_COLUMNS=${KEEP_LIST_COLUMNS:-0}
firstcol() {
  if [ "$KEEP_LIST_COLUMNS" = 1 ]; then FIRSTCOL=$1; return; fi
  case $1 in
    '"'*) f_rest=${1#\"}                       # a quoted first field
          FIRSTCOL="\"${f_rest%%\"*}\"" ;;
    *)    FIRSTCOL=${1%%,*} ;;
  esac
}
...
# Render a ';'-joined collection as a JSON array, so the column parses as a
# real list (json.loads / ast.literal_eval) instead of a delimited string.
# An empty collection becomes [], never an empty string, so the column's type
# stays consistent across rows.  JLIST holds the result.
jlist() {
  j_in=$1; JLIST="["
  if [ -n "$j_in" ]; then
    j_first=1
    while [ -n "$j_in" ]; do
      ...
      [ -n "$j_it" ] || continue
      j_it=${j_it//'\\'/'\\\\'}; j_it=${j_it//'"'/'\\"'}
      if [ "$j_first" = 1 ]; then JLIST="$JLIST\"$j_it\""; j_first=0
      else JLIST="$JLIST,\"$j_it\""; fi
    done
  fi
  JLIST="$JLIST]"
}
...
while IFS="$(printf '\t')" read -r i name ver; do
  ...
  if [ -z "$ver" ]; then
    ...
    # Licenses are structured objects, not just key names, so they get their
    # own parser; everything else is a flat list of keys.
    [ "$WANT_LICENSES" = 1 ] && { lic=$(awk -f "$TMP/licenses.awk" "$vy" 2>/dev/null) || {
  ...
  # Licenses arrive as JSON already; the rest are ';'-joined and become arrays.
  [ "$WANT_LICENSES" = 1 ]          && { csvq "$lic";  row="$row,$CSVQ"; }
  [ "$WANT_CATALOGS" = 1 ]          && { jlist "$cat";  csvq "$JLIST"; row="$row,$CSVQ"; }
  [ "$WANT_CERTIFICATIONS" = 1 ]    && { jlist "$cert"; csvq "$JLIST"; row="$row,$CSVQ"; }
  [ "$WANT_CLASSIFICATIONS" = 1 ]   && { jlist "$cls";  csvq "$JLIST"; row="$row,$CSVQ"; }
  [ "$WANT_ARCHITECTURES" = 1 ]     && { jlist "$arch"; csvq "$JLIST"; row="$row,$CSVQ"; }
  [ "$WANT_K8S_DISTROS" = 1 ]       && { jlist "$k8s";  csvq "$JLIST"; row="$row,$CSVQ"; }
  [ "$WANT_MANAGED_PLATFORMS" = 1 ] && { jlist "$mp";   csvq "$JLIST"; row="$row,$CSVQ"; }
