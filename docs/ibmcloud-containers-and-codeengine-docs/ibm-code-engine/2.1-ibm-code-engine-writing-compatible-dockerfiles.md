...
name: codeengine-dockerfiletips
title: Writing a Dockerfile for Code Engine
description: Before you build your code into a container image, learn some basics of how a Docker build works within IBM Cloud&reg; Code Engine. Then, look at some best practices for your Dockerfile to reach these goals. These examples focus on reducing the image size, improving the interoperability with Code Engine applications, and to run containers as a non-root user.
...
If you copy the entire Git repository, but want to exclude some files, for example the `README.md` of the repository, then you can add a [`.dockerignore` file](https://docs.docker.com/reference/dockerfile/#dockerignore-file){: external}. Use the same file to also ignore the files and directories that you specify in your [.gitignore file](https://git-scm.com/docs/gitignore){: external}. By using the same file, you ensure that a build that you run locally has the same set of files available as the build running in Code Engine.
{: tip}

Always copy your application files into a subdirectory of the root (`/`) rather than into the root directly to avoid conflicts with operating system files. When you name your application directory, do not use one that is reserved by UNIX-based operating systems, Kubernetes or Code Engine builds, such as `/bin`, `/dev`, `/etc`, `/lib`, `/proc`, `/run`, `/sys`, `/usr`, `/var`, or `/workspace`. Naming your application directory `/app` is a best practice.
{: note}

If your source code repository contains the sources for different applications that are organized in directories, similar to the [Code Engine samples repository](https://github.com/IBM/CodeEngine){: external}, then you can use a subdirectory as your context. In the [**`ibmcloud ce build create`**](https://cloud.ibm.com/docs/codeengine?topic=codeengine-cli#cli-build-create) command, specify subdirectories by using the `--context-dir` option.

If your Dockerfile is not in the context directory, then you can point to it by using the `--dockerfile` argument.

To try out a Docker build on your local system before you build it in Code Engine, you can use [Docker Desktop](https://www.docker.com/products/docker-desktop){: external}.
...
- The application or job that uses the image starts faster because the time that is needed to pull the image is shorter. As the resources that are needed to run your application or job are reserved while the image is pulled, you again save money. A fast startup time is especially relevant for applications in Code Engine because it guarantees an acceptable response time for the requests of your users, even if the application is scaled down to zero.
